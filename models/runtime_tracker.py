...
from models import build_model, build_xy_pe, build_xywh_pe, build_frame_pe
...
class RuntimeTracker:
    def __init__(self, config, model):
        ...
        self.xywh_pe = build_xywh_pe(config=config)
        ...
        self.bos_xywh_pos = torch.zeros((1, config["DIM"] * 2), dtype=torch.float32, device=self.device)
        self.bos_xywh = torch.zeros((1, 4), dtype=torch.float32, device=self.device)
        ...
        self.trk_xywh_pos = torch.zeros((0, config["DIM"] * 2), dtype=torch.float32, device=self.device)
        self.trk_xywh = torch.zeros((0, 4), dtype=torch.float32, device=self.device)
    ...
    def update(self, img, dets, orig_dets):
        ...
        whs = dets[:, 2:4] - dets[:, :2]
        normed_box = dets.clone()[:, :4]
        normed_box[:, :2] = (dets[:, :2] + dets[:, 2:4]) / 2.
        normed_box[:, 2:4] = whs
        
        normed_box[:, ::2] /= img_w + 1e-6
        normed_box[:, 1::2] /= img_h + 1e-6
        
        cxcywhs = normed_box[:, :4]
        dets_xywh = cxcywhs
        dets_xywh_pos = self.xywh_pe(cxcywhs)
        
        dets_frame_pos = self.frame_pos[len(self.ntrk_per_frame)].unsqueeze(0).repeat(dets.shape[0], 1)
        
        dets_iou, o = self.hmiou(dets[:, :4], dets[:, :4])
        dets_iou *= o
        ...
        if self.trks.shape[0] > 0:
            ...
            inputs_xywh_pos = torch.cat((self.bos_xywh_pos, self.trk_xywh_pos, dets_xywh_pos), dim=0).unsqueeze(0)
            inputs_xywh = torch.cat((self.bos_xywh, self.trk_xywh, dets_xywh), dim=0).unsqueeze(0)
            inputs_frame_pos = torch.cat((self.bos, self.trk_frame_pos, dets_frame_pos), dim=0).unsqueeze(0)
            
            output_tokens = self.model(input_tokens, inputs_frame_pos, inputs_xywh_pos, atten_mask, is_projected=True)
            ...
            abs_iou = self.abs_iou(inputs_xywh[0], inputs_xywh[0, self.start_coord[-1]:])
            cost_matrix *= abs_iou
            ...
            for i, trk_id in enumerate(trk_ids):
                cost_matrix_f[i] = cost_matrix[self.id_coords[trk_id][:, -1]].max(dim=0)[0]
            
            cost_matrix_box, o = self.hmiou(self.trks[:, TI.TLBR:TI.TLBR + 4], dets[:, :4])
            cost_matrix_f *= o
            cost_matrix_box *= o
            final_cost_matrix = (cost_matrix_f + cost_matrix_box) * o * dets[:, 4].unsqueeze(0) * self.trks[:, TI.Score].unsqueeze(1)
            ...
            if matches.shape[0] != 0:
                ...
                matched_xywh_pos = dets_xywh_pos[m1, :]
                matched_xywh = dets_xywh[m1, :]
                ...
                matched_xywh_pos = torch.zeros((0, self.dim * 2), dtype=torch.float32, device=self.device)
                matched_xywh = torch.zeros((0, 4), dtype=torch.float32, device=self.device)
        ...
        if len(self.ntrk_per_frame) > self.max_nframes:
            ...
            self.trk_xywh_pos = self.trk_xywh_pos[pop_n:]
            self.trk_xywh = self.trk_xywh[pop_n:]
        ...
        self.trk_xywh_pos = torch.cat([self.trk_xywh_pos, matched_xywh_pos, dets_xywh_pos[undets, :][t]], dim=0)
        self.trk_xywh = torch.cat([self.trk_xywh, matched_xywh, dets_xywh[undets, :][t]], dim=0)
    ...
    def hmiou(self, bboxes1, bboxes2):
        ...
        iou = area / ((bboxes1[..., 2] - bboxes1[..., 0]) * (bboxes1[..., 3] - bboxes1[..., 1])
                   + (bboxes2[..., 2] - bboxes2[..., 0]) * (bboxes2[..., 3] - bboxes2[..., 1]) - area)
        
        return iou, o
    
    def abs_iou(self, boxes1, boxes2):
        ...
        Args:
            boxes1: (torch.Tensor[N, 4]): first set of xywh boxes
            boxes2: (torch.Tensor[M, 4]): second set of xywh boxes
        ...
        cx, cy, w, h = b1.unbind(-1)
        b1 = [(cx - 0.5 * w), (cy - 0.5 * h), (cx + 0.5 * w), (cy + 0.5 * h)]
        b1 = torch.stack(b1, dim=-1)
        cx, cy, w, h = b2.unbind(-1)
        b2 = [(cx - 0.5 * w), (cy - 0.5 * h), (cx + 0.5 * w), (cy + 0.5 * h)]
