...
from models import build_xy_pe, build_frame_pe
...
class RuntimeTracker:
    ...
    def update(self, img, dets, orig_dets):
        ...
        if self.trks.shape[0] > 0:
            ...
            atten_mask = self.get_atten_mask(input_tokens)
            ...
            output_tokens = self.model(input_tokens, inputs_frame_pos, inputs_xy_pos, atten_mask, is_projected=True).squeeze(0)
            ...
            cost_matrix_f = torch.zeros((len(trk_ids), dets.shape[0]), dtype=torch.float32, device=self.device)
    ...
    def get_atten_mask(self, input_tokens):
    ...
    @staticmethod
    def associate(final_cost_matrix, cost_matrix_box, cost_matrix_f, threshold_1=0.1, threshold_2=0.1):
