...
def train_one_epoch(config: dict, lrs, xy_pe, xywh_pe, frame_pe, model, train_states: dict, max_norm: float,
    ...
    for _i, batch in enumerate(dataloader):
        ...
        for bidx in range(batch_size):
            ...
            for fidx in range(n_frames):
                ...
                if n_boxes[bidx, fidx] > 0:
                    # xyxy -> cxcywh and normalize
                    whs = box[:, 2:] - box[:, :2]
                    normed_box = box.clone()
                    normed_box[:, :2] = (box[:, :2] + box[:, 2:]) / 2.
                    normed_box[:, 2:] = whs
                    
                    normed_box[:, ::2] /= img_w + 1e-6
                    normed_box[:, 1::2] /= img_h + 1e-6
                    
                    temp_cxcywhs.append(normed_box)
                    ...
                    temp_cxcywhs.append(torch.zeros((0, 4), device=device, dtype=torch.float32))
            ...
            cxcywhs.append(temp_cxcywhs)
            gts.append(temp_gts)
            _col_gts_coords.append(temp__col_gts_coords)
        del temp_sfeatures_list, temp_cxcywhs, temp_img_hws, temp_gts, temp__col_gts_coords
        ...
        inputs_xywh_pos = torch.zeros(batch_size, 1 + max_nbox_per_frame.sum(), dim * 2, device=device)
        inputs_xywh = torch.zeros(batch_size, 1 + max_nbox_per_frame.sum(), 4, device=device)
        ...
        for fidx in range(n_frames):
            for bidx in range(batch_size):
                ...
                cxcywh = cxcywhs[bidx][fidx]
                if cxcywh.shape[0] > 0:
                    inputs_xywh[bidx, start_coord[fidx]:start_coord[fidx] + n_boxes[bidx, fidx]] = cxcywh
                    inputs_xywh_pos[bidx, start_coord[fidx]:start_coord[fidx] + n_boxes[bidx, fidx]] = xywh_pe(cxcywh)
        ...
        pred_logits = model(inputs, inputs_frame_pos, inputs_xywh_pos, atten_mask)
        del inputs, inputs_frame_pos, inputs_xy_pos, inputs_xywh_pos, inputs_id_emb_idxs, atten_mask
        
        avg_box_per_frame = n_boxes.sum(axis=0).float()
        if is_distributed():
            torch.distributed.all_reduce(avg_box_per_frame)
        avg_box_per_frame = torch.clamp(avg_box_per_frame / (distributed_world_size() * n_boxes.shape[0]), min=1).tolist()
        
        loss = 0.
        for bidx in range(batch_size):
            for _fidx in range(n_frames - 1):
                class_avg_box = avg_box_per_frame[_fidx]
                gt_matcher = gts[bidx][_fidx].to(device)
                if gt_matcher.shape[0] == 0:
                    continue
                
                affinity_matrix = pred_logits[:, bidx, col_gts_coords[bidx][_fidx]] @ pred_logits[:, bidx, start_coord[_fidx + 1]:start_coord[_fidx + 1] + n_boxes[bidx, _fidx + 1]].permute(0, 2, 1)
                logits = affinity_matrix.reshape(-1, affinity_matrix.shape[-1])
                gt_class = gt_matcher.repeat(pred_logits.shape[0])
                loss_class = CEloss(logits, gt_class) / class_avg_box
                
                loss += loss_class / pred_logits.shape[0]
        del gt_matcher, affinity_matrix
