...
def read(path):
    ...
    print(gt_path_v2)
    if "MOT17" in path:
        img_path = os.path.join(f'./datasets/MOT17/test/{path}', 'img1')
    else:
        img_path = os.path.join(f'./datasets/MOT20/test/{path}', 'img1')
    ...
    for i in range(1, 4000):
        ...
        #     return 0
        img = cv2.imread(img_path + '/' + '0' * (6 - len(str(i))) + str(i) + '.jpg')
        if img is None:
            break
